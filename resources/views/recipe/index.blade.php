                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-500">
                    ...
                    <td class="px-6 py-3">
                        @foreach ($recipe->themes as $theme)
                            <div class="text-xs text-center text-gray-900 bg-{{ $theme->color }}-500 py-1 px-2 rounded-full">{{$theme->name}}</div>
                        @endforeach
                    </td>
                    <td class="flex items-center gap-2 px-6 py-3">
                        <a  href="{{ route('recipes.show', $recipe) }}"
                        class="rounded-md border border-gray-600 text-sm text-gray-600 py-2 px-3 hover:bg-gray-200 hover:text-gray-400 cursor-pointer transition-all duration-200">Voir</a>
