    ...
    <x-slot name="header">
        ...
        <div class="relative overflow-x-auto mt-12 max-w-xl mx-auto px-8">
            <form
                ...
                action="{{ route('step.update', $step) }}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <input class="flex items-center h-8 px-4 w-48 bg-gray-200 mt-2 rounded focus:outline-none focus:ring-2">
                {{-- type="text" name="id_ingredient" value="{{ $step->ingredient }}"> --}}

                <label class="font-semibold mt-3 text-gray-500" for="type">Status :</label>
                <input class="flex items-center h-8 px-4 w-48 bg-gray-200 mt-2 rounded focus:outline-none focus:ring-2"
                    type="text" name="status" value="{{ $step->status }}">
                <button
                    class="flex items-center justify-center h-8 px-2 w-36 bg-blue-600 mt-8 rounded font-semibold text-sm text-blue-100 hover:bg-blue-700">Modifier</button>
            </form>
        </div>
