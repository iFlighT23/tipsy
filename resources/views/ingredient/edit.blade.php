<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <a href="{{ route('ingredient.index') }}" class="border border-green-600 text-green-600 rounded-md py-2 px-2">Retour aux ingrédients</a>
    </x-slot>
...
<div class="relative overflow-x-auto mt-12 max-w-xl mx-auto px-8">
    <form class="flex flex-col text-sm text-gray-500 uppercase bg-gray-50 rounded shadow-lg p-12 mt-12 dark:bg-gray-700 dark:text-gray-400"
        action="{{ route('ingredient.update', $ingredient) }}" method="POST">
        @csrf
        @method('PATCH')
